...
const App = ({ nameCategory, idCategory, updateData }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://localhost/riwayat-asset/category/${idCategory}`);
  ...
  return (
    ...
    <Tooltip title="Lihat Riwayat">
      ...
      <Drawer width={660} placement="right" closable={false} onClose={onClose} open={open}>
        ...
        <Divider />
        <p className="site-description-item-profile-p">Riwayat : <b>{nameCategory}</b></p>
        <Divider />
